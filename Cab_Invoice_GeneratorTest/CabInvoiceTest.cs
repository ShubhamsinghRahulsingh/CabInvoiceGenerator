...
namespace Cab_Invoice_GeneratorTest
{
    public class CabInvoiceTest
    {
        ...
        public void GivenRideOption_NormalOrPremium_ReturnInvoiceSummary()
        {
            ...
            repository.MapRidesToUser("akshay", ride);
            repository.MapRidesToUser("tejas", rideNew);
            EnhancedInvoiceSummary INVOICE_SUMMARY = repository.GetEnhancedInvoiceSummary(RideOption.RideTypes.NORMAL,"akshay");
            EnhancedInvoiceSummary INVOICE_SUMMARY_NEW = repository.GetEnhancedInvoiceSummary(RideOption.RideTypes.PREMIUM,"tejas");
            Assert.AreEqual(75, INVOICE_SUMMARY.totalFare);
            Assert.AreEqual(25, INVOICE_SUMMARY.averageFarePerRide);
            Assert.AreEqual(3, INVOICE_SUMMARY.totalNumberOfRides);
